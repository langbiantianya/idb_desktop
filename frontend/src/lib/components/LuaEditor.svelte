<script>
	...
	const BUILTINS = [
		...
		{ label: 'random_date', detail: 'random_date(start, end)', insertText: "random_date('${1:1970-01-01}', '${2:2005-12-31}')", doc: 'Random LocalDate in [start, end] (YYYY-MM-DD) — binds via JDBC DATE' },
		{ label: 'random_datetime', detail: 'random_datetime(start, end)', insertText: "random_datetime('${1:2024-01-01}', '${2:2026-12-31}')", doc: 'Random LocalDateTime in [start, end] (second precision) — binds via JDBC TIMESTAMP' },
		{ label: 'random_time', detail: 'random_time()', insertText: 'random_time()', doc: 'Random LocalTime (HH:mm:ss) — binds via JDBC TIME' },
