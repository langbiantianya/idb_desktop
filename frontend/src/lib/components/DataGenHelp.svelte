...
<Modal {open} {onClose} title={$t('dg.help_title')} size="lg">
	<div class="dg-help-content">
		...
		<h4>{$t('dg.help.globals')}</h4>
		<table>
			<thead><tr><th>{$t('dg.help.var')}</th><th>{$t('dg.help.desc')}</th></tr></thead>
			<tbody>
				<tr><td><code>count</code></td><td>{@html $t('dg.help.count.desc')}</td></tr>
			</tbody>
		</table>
		...
		<pre class="dg-json"><code>{"{\"tables\":[{\"count\":100,\"script\":\"for i = 1, count do\\n  insert('users', {\\n    name = 'user_' .. i,\\n    email = 'user_' .. i .. '@test.com'\\n  })\\nend\"}]}"}</code></pre>
